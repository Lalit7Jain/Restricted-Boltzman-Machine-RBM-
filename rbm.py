...
def rbm(dataset, num_hidden, learn_rate, epochs, batchsize):
   ...
   Train a Restricted Boltzmann Machine where input can be either binary or real-valued in the interval [0,1].
   For training, we split the dataset into several smaller mini-batches. We also use momentum when updating weights.
   ...
   for epoch in range(epochs):
      ...
      for batch in range(batches):
         ...
         # activation values needed to update biases (axis = 0, along the columns)
...
def reconstruct(v0, w, a, b):
   ...
   Reconstruct the input vector v0 by sampling hidden states and then sampling visible states from the hidden states.
   This is useful for measuring the reconstruction error, which can be viewed as a goodness of learned RBM.
...
def sample_hidden(v0,w,b):
   ...
   Sample a vector of hidden units given the visible vector and the trained weights. This function is used to convert
   the initial representation into the representation given  by hidden units.
...
def test_mnist(n_examples, num_hidden, epochs, learn_rate):
   ...
   Load train set images, then train RBM, then use several test set images to measure the  reconstruction error on the unseen data points.
   Finally, save the reconstructed images and learned weights in the "Output" folder.
